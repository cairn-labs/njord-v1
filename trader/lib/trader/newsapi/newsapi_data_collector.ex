defmodule Trader.Newsapi.NewsapiDataCollector do
      ...
      {:ok, item} ->
        proto =
          DataPoint.new(
            ...
            news_api_item: %NewsApiItem{item |
                                        country: country_code,
                                        search_type: :TOP_HEADLINES}
          )
        [proto]
  ...
  defp retrieve_search_results(query) do
    ...
    window_length_ms = round(86400000 / (calls_per_day / length(@searches))) * 10
  ...
  defp queue_next_tick(pid) do
    calls_per_day = Application.get_env(:trader, __MODULE__)[:max_calls_per_day]
    delay = round(86400000 / (calls_per_day / length(@searches)))
